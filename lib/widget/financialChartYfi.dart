...
class WidgetFinancialChartYfi extends StatefulWidget {
  const WidgetFinancialChartYfi({Key? key, required this.yfiPredict})
      : super(key: key);
  final dynamic yfiPredict;
  @override
  _WidgetFinancialChartState createState() => _WidgetFinancialChartState();
}

class _WidgetFinancialChartState extends State<WidgetFinancialChartYfi> {
  ...
  @override
  void initState() {
    for (var i = 0; i < widget.yfiPredict[0].length; i++) {
      //print(widget.yfiPredict[0][0]);
      //print(double.parse(widget.yfiPredict[0][i.toString()]['next_predicted_days_value'].toString()));
      chartData.add(ChartData(
          widget.yfiPredict[0][i]['date'].toString(),
          double.tryParse(
              widget.yfiPredict[0][i]['original_close'].toString())));
    }
    for (var i = 0; i < widget.yfiPredict[0].length; i++) {
      predictData.add(ChartDataPredict(
          widget.yfiPredict[0][i]['date'].toString(),
          double.tryParse(
              widget.yfiPredict[0][i]['test_predicted_close'].toString())));
    }
