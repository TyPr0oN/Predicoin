...
class BacktestResultPaxgRsiPage extends StatefulWidget {
  const BacktestResultPaxgRsiPage({Key? key, required this.investresultRsi})
      : super(key: key);
...
class _BacktestResultPageState extends State<BacktestResultPaxgRsiPage> {
  File? backtestPaxg;
  ...
  Future<String> postInvest(investValue) async {
    try {
      postInvestResult = await Provider.of<BacktestPaxg>(context, listen: false)
          .sendInvestPaxgRsi(investment_value: investValue);
      PaxgInfo =
          await Provider.of<CoinPaxg>(context, listen: false).fetchRequest();
      print('hellotry from RSI paxg');
                              ...
                              children: [
                                InteractiveViewer(
                                  ...
                                  child: new Image.network(
                                    'http://10.0.2.2:8000/paxg_rsi_plot',
                                    // width: 400,
                                    // height: 400,
                                    fit: BoxFit.cover,
                                  ),
