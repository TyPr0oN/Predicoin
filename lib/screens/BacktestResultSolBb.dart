...
class BacktestResultSolBbPage extends StatefulWidget {
  const BacktestResultSolBbPage({Key? key, required this.investresultBb})
      : super(key: key);
...
class _BacktestResultPageState extends State<BacktestResultSolBbPage> {
  File? backtestSol;
  ...
  Future<String> postInvest(investValue) async {
    try {
      postInvestResult = await Provider.of<BacktestSol>(context, listen: false)
          .sendInvestSolBb(investment_value: investValue);
      solinfo =
          await Provider.of<CoinSol>(context, listen: false).fetchRequest();
      print('hellotry from bb sol');
                              ...
                              children: [
                                InteractiveViewer(
                                  ...
                                  child: new Image.network(
                                    'http://10.0.2.2:8000/sol_bb_plot',
                                    // width: 400,
                                    // height: 400,
                                    fit: BoxFit.cover,
                                  ),
