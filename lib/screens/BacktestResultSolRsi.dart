...
class BacktestResultSolRsiPage extends StatefulWidget {
  const BacktestResultSolRsiPage({Key? key, required this.investresultRsi})
      : super(key: key);
...
class _BacktestResultPageState extends State<BacktestResultSolRsiPage> {
  File? backtestSol;
  ...
  Future<String> postInvest(investValue) async {
    try {
      postInvestResult = await Provider.of<BacktestSol>(context, listen: false)
          .sendInvestSolRsi(investment_value: investValue);
      SolInfo =
          await Provider.of<CoinSol>(context, listen: false).fetchRequest();
      print('hellotry from RSI sol');
                              ...
                              children: [
                                InteractiveViewer(
                                  ...
                                  child: new Image.network(
                                    'http://10.0.2.2:8000/sol_rsi_plot',
                                    // width: 400,
                                    // height: 400,
                                    fit: BoxFit.cover,
                                  ),
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child: Text(
                                  "Maximum Drawdown : " +
                                      '${(double.parse((SolInfo[0]['${SolInfo[0].length - 1}']['Max_dd']).toString()) * 100).toStringAsFixed(2)}' +
                                      " % ",
                                  style: TextStyle(
                                      fontFamily: 'Ruda',
                                      fontSize: 15,
