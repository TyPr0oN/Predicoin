...
class BacktestResultEthMacdPage extends StatefulWidget {
  const BacktestResultEthMacdPage({Key? key, required this.investresultMacd})
      : super(key: key);
...
class _BacktestResultPageState extends State<BacktestResultEthMacdPage> {
  File? backtestEth;
  ...
  Future<String> postInvest(investValue) async {
    try {
      postInvestResult = await Provider.of<BacktestEth>(context, listen: false)
          .sendInvestEthMacd(investment_value: investValue);
      ethinfo =
          await Provider.of<CoinEth>(context, listen: false).fetchRequest();
      print('hellotry from macd eth');
                              ...
                              children: [
                                InteractiveViewer(
                                  ...
                                  child: new Image.network(
                                    'http://10.0.2.2:8000/eth_macd_plot',
                                    // width: 400,
                                    // height: 400,
                                    fit: BoxFit.cover,
                                  ),
