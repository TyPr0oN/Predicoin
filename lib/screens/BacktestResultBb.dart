...
class BacktestResultBbPage extends StatefulWidget {
  const BacktestResultBbPage({Key? key, required this.investresultBb})
      : super(key: key);
...
class _BacktestResultPageState extends State<BacktestResultBbPage> {
  File? backtestBtc;
  ...
  Future<String> postInvest(investValue) async {
    try {
      postInvestResult = await Provider.of<BacktestBtc>(context, listen: false)
          .sendInvestBtcBb(investment_value: investValue);
      btcinfo =
          await Provider.of<CoinBtc>(context, listen: false).fetchRequest();
      print('hellotry from bb btc');
                              ...
                              children: [
                                InteractiveViewer(
                                  ...
                                  child: new Image.network(
                                    'http://10.0.2.2:8000/btc_bb_plot',
                                    // width: 400,
                                    // height: 400,
                                    fit: BoxFit.cover,
                                  ),
